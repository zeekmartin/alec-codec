...
use alec_testdata::{generate_dataset, Dataset, DatasetManifest, GeneratorConfig, SensorManifest};
...
fn generate_satellite_datasets() {
    ...
    let scenarios = [
        (
            ...
            "satellite_stationary_24h",
            24.0,
        ),
        (
            SatelliteScenario::MovingAsset,
            "satellite_moving_asset_8h",
            8.0,
        ),
        (
            SatelliteScenario::BatteryCritical,
            "satellite_battery_critical",
            12.0,
        ),
...
fn generate_manufacturing_datasets() {
    ...
    let scenarios = [
        (
            ...
            "factory_normal_shift_8h",
            8.0,
        ),
        (
            ManufacturingScenario::MachineCycle,
            "factory_machine_cycle_1h",
            1.0,
        ),
        (
            ManufacturingScenario::BearingFailure,
            "factory_bearing_failure",
            4.0,
        ),
...
fn create_manifest(
    ...
    for sensor in sensors {
        if let Some(stats) = dataset.stats(&sensor.id) {
            manifest = manifest.add_sensor(SensorManifest::new(
                &sensor.id,
                &sensor.unit,
                stats.min,
                stats.max,
